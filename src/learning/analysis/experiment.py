...
from src.learning.generate.generate_synthetic_graph import (
    GraphParams,
    generateSyntheticBipartiteGraph,
    get_graph_metric,
)
...
from src.learning.models.loss.hyperplane import HYPERPLANE
from src.learning.models.loss.transE import TRANS_E
...
def run_experiment_on_one_graph(data_path, n_params):
    ...
    # todo: add ordered embeddings


def get_experiment_methods():
    return [
        TRANS_E().to_string(),
...
def generate_graphs_for_experiment(test_param, save_dir=None):
    ...
    for i in BIPARTITE_DEFAULT_RANGE[test_param]:
        ...
        g, adj_matrix, interaction_matrix, data_path = generateSyntheticBipartiteGraph(
            params, save_graph=True, save_dir=save_dir
        )
        data_paths.append(data_path)
    return data_paths


def generate_graphs_for_experiment_two_params(
    test_param_1, test_param_2, save_dir=None
):
    data_paths = []
    for i in BIPARTITE_DEFAULT_RANGE[test_param_1]:
        for j in BIPARTITE_DEFAULT_RANGE[test_param_2]:
            params = GraphParams(
                {**BIPARTITE_DEFAULT_PARAMS, test_param_1: i, test_param_2: j}
            )
            (_, _, _, data_path,) = generateSyntheticBipartiteGraph(
                params, save_graph=True, save_dir=save_dir
            )
            data_paths.append(data_path)
    return data_paths
...
def collect_experiment_results(data_dir):
    ...
    for data_path in exp_roots:
        ...
        for n_param in os.listdir(os.path.join(data_path, "results")):
            if not os.path.isdir(os.path.join(data_path, "results", n_param)):
                continue
            model_best_F1 = get_models_best_F1(
                os.path.join(data_path, "results", n_param)
            )
            n_param = int(n_param.split("=")[1])
            experiment_results_df.append(
                {**graph_metric, **model_best_F1, "n_param": n_param}
            )
...
def get_models_best_F1(result_dir):
    ...
    for model in os.listdir(result_dir):
        if not os.path.isdir(os.path.join(result_dir, model)):
            continue
            ...
            if os.path.isdir(os.path.join(result_dir, model, trial)):
                ...
                for file in os.listdir(os.path.join(result_dir, model, trial)):
                    if file.endswith(".json"):
                        metrics = json.load(
                            open(os.path.join(result_dir, model, trial, file), "r")
                        )
